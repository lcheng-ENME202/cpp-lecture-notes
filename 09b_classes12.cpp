...
Complex cget(void) {
  Complex z;
  cin >> z.real >> z.imag;
  return z; // we are allowed to return a single variable... now this single
            // variable happens to contain multiple things, but it's still one
            // variable!
}
...
void cdisplay(Complex z) {
  ...
  if (z.imag > 0) {
    ...
    cout
        << z.imag
        << "j"; // remember that the minus sign is printed automatically here...
  }
  cout << endl;
}
